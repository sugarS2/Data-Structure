...
/**
 * 참고URL : https://st-lab.tistory.com/183
 */
public class ArrayQueue<E> implements QueueInterface<E> {
	...
	// 사용법 : ArrayQueue<Integer> q = new ArrayQueue<>();
	public ArrayQueue() {
	...
	// 생성자2 (초기 용적 할당을 할 경우)
	// 사용법 : ArrayQueue<Integer> q = new ArrayQueue<>(100);
	...
	public boolean offer(E item) {
		...
		if( (rear+1) % array.length == front ) {
			resize(array.length * 2); // 용적을 2배 늘려준다.
		}
	...
	public E poll() {
		...
		if(size == 0) { // 삭제할 요소가 없는 경우 null 반환
			return null;
		}
		...
		// 용적이 최소크기(64)보다 크고 요소 개수가 1/4 미만일 경우
	...
	/* 
	 * 가장 앞에 있는 데이터((front+1) % array.length)를 삭제하지 않고 확인만 하고 싶을 경우에 사용
	 */
	@Override
	public E peek() {
	...
	/**
	 * 현재 큐에 있는 요소의 개수를 반환하는 메소드입니다.   
	 * @return 현재 큐에 있는 요소의 개수를 반환
	 */
	public int size() {
		return size;
	}
	...
	/**
	 * 현재 큐가 비어있는지를 확인합니다. 
	 * @return 비어있다면 {@code true}, 비어있지 않다면 {@code false}를 반환
	 */
	...
	/**
	 * 현재 찾고자 하는 요소가 큐에 들어가있는지 알려주는 메소드입니다. 
	 * @param value 찾고자 하는 요소
	 * @return 큐에 들어가있으면 {@code true}, 들어있지 않으면 {@code false} 반환
	 */
